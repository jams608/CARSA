...
def train(args, train_dataset, model, eval_dataset):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            inputs['epoch'] = epoch
            inputs['max_epoch'] = int(args.num_train_epochs)
...
def get_input_from_batch(args, batch):
    inputs = {'a_input_ids': batch[0].to(args.device),
            ...
            'sentiment_token_labels': batch[11].to(args.device),
            'cap_input_ids': batch[12].to(args.device),
            'cap_attention_mask': batch[13].to(args.device),
            'imgid': batch[14].to(args.device),
            'long_cap_input_ids': batch[15].to(args.device),
            'long_cap_attention_mask': batch[16].to(args.device),
            'caption_input_ids': batch[17].to(args.device),
            'caption_attention_mask': batch[18].to(args.device),
            }
